...
def load_and_process_data(file_path):
    with open(file_path, 'r') as f:
        data = json.load(f)
    return pd.DataFrame([parse_record(r) for r in data])
...
#
if len(labeled_remarks) >= 5: # if a train model have atleast 5 samples
    X_train, y_train = zip(*labeled_remarks) # zip * means get a list of pairs into two list the split into x and into y training dataset
    model = make_pipeline(TfidfVectorizer(), LogisticRegression(max_iter=200)) #make pipeline creates machine learning pipeline to chain together
    #multiple steps into one pipeline
    #Term Frequency , IDF Inverse document frequency, how often does a word comes, and check for the word all across the document, score based on it

    #thehn apply model
    model.fit(X_train, y_train)
    model_ready = True # the model has loaded initialization
else:
    model_ready = False # if model has not finished loading
...
if machine_input:
    try:
        ...
        if df_mach.empty:
            st.warning(f"No data found for machine number {machine_no}.")
        else:
            ...
            # Check if batch has more than one lot
            ...
            if remarks_list and 'model' in globals():
            ...
            if len(X) > 2:
                ...
                with col1:
                    fig1 = go.Figure()
                    fig1.add_trace(go.Scatter(x=df_mach_sorted["Creation_Date"], y=y_moisture, mode='lines+markers', name='Actual Moisture'))
                    fig1.add_trace(go.Scatter(x=future_dates, y=future_moisture, mode='lines', name='Forecast Moisture', line=dict(color='red')))
                ...
                with col4:
                    fig3 = go.Figure()
                    fig3.add_trace(go.Scatter(x=df_mach_sorted["Creation_Date"], y=y_speed, fill='tozeroy', mode='none', name='Actual Speed', fillcolor='lightblue'))
                    fig3.add_trace(go.Scatter(x=future_dates, y=future_speed, fill='tonexty', mode='none', name='Forecast Speed', fillcolor='lavender'))
            ...
            with col10:
                st.write(f"Speed (M/Min): {latest['Speed _MMin']}")
                fig9 = go.Figure()
